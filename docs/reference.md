...
| `cooldownHours` | `1` | 模型报额度问题后晾多久再试。文案给的恢复时间更长时才按文案的来 |

**没写的字段用默认值，不用把整份配置抄一遍。** 数字字段填成非数字（`"abc"`、`null`、`""`）也会被忽略、改用默认值——`/rl doctor` 会明确列出这些字段，不会让你以为它生效了。
...
|---|---|---|
| `path` | ✅ | 盯这个目录下有没有 `DONE` |
| `pid` | 可选 | 判崩溃。填了崩了能**立刻**发现；不填只能靠「目录不产出了」判断，约 `stallMinutes` 后提醒 |
...
**默认 1 小时**，不去猜 provider 说的恢复时间。

各家文案措辞五花八门（`Try again in ~192 min` 只有部分 provider 有），抠不准；
而一次失败调用的代价很低（额度错误不消耗 token），所以用**低频重试**比
「精确算恢复时间」更简单也够用。

唯一例外：文案里明确给出**比 1 小时更长**的恢复时间时按它的来，
免得 1 小时后白跑一趟。给得更短（30s 那种）也不跟着缩短。
